...
tags = {
	 names = {"TERM", "DEV", "DOC", "TEST", "CHAT", "WEB", "FUN", "XTRA"}
}
for s = 1, screen.count() do
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey,		  }, "k",     function () awful.util.spawn("dm-tool lock") end),
    ...
    awful.key({ modkey, "Shift"   }, "Right", function () awful.util.spawn("xmms2 next") end),
    awful.key({ }, "XF86AudioNext", 	      function () awful.util.spawn("xmms2 next") end),
    awful.key({ modkey, "Shift"   }, "Up",    function () awful.util.spawn("xmms2 server volume +10") end),
    awful.key({ modkey, "Shift"   }, "Down",  function () awful.util.spawn("xmms2 server volume -10") end),
    awful.key({ modkey, "Shift"   }, "p",     function () awful.util.spawn("xmms2 toggle") end),
    awful.key({ }, "XF86AudioPlay",	      function () awful.util.spawn("xmms2 toggle") end),
    awful.key({ }, "XF86AudioRaiseVolume",    function () awful.util.spawn("amixer set Master 9%+ & amixer sset Master toggle") end),
    awful.key({ }, "XF86AudioLowerVolume",    function () awful.util.spawn("amixer set Master 9%- & amixer sset Master toggle") end),
    awful.key({	}, "XF86AudioMute", 	      function () awful.util.spawn("amixer sset Master toggle") end),
    awful.key({ modkey,           }, "b",     function () batteryShow("BAT0") end,"Battery"),
...
function batteryInfo(adapter)
     ...
     battery_widget:set_markup(dir..battery.." | ")
     fcur:close()
     fcap:close()
     fsta:close()
 end
...
-- Brightness update function (obsolete)
function update_brightness(update)
	--local mbfd = io.popen("/sys/class/backlight/intel_backlight/max_brightness")
	local cbfd = io.popen("/sys/class/backlight/intel_backlight/brightness")
	--local mb = math.floor(mbfd:read())
	local cb = math.floor(cbfd:read())
	local change = math.floor(update)
	...
	io.close(cbfd)
	cbfd = iopopen("/sys/class/backlight/intel_backlight/brightness", "w+")
	cbfd:write(cb)
	io.close(cbfd)
end
